---
name: research
description: Research high-trust sources and produce a cited self-contained HTML report with evidence quality, recursive critique loops, coverage gates, strict closure records, and downstream decision-gate JSON. Use when the user asks to ??, ????, ?????, ????, ????/API, ????, ??????, ????, License/CVE/security review, PRD/test strategy references, Agent/Skill design references, grill finding evidence support, or public test URL research.
---
...
- Never bypass login, paywalls/terms, robots, or anti-bot controls.
...
- For high-impact reports, record the recursive evidence trail explicitly: source-review findings, follow-up queries, and P0/P1 closure reasons must be machine-readable, not implied only in prose.
...
8. For `standard` and `deep` work, run the evidence critique loop in `references/evidence-critique-loop.md`: review strengths, weaknesses, confidence, P0/P1 gaps, and follow-up queries; emit `critique-loop-log`, `source-review-findings`, `followup-query-matrix`, and `p0p1-closure-matrix`; repeat until P0/P1 gaps are closed, blocked, budget-exhausted, or explicitly risk-accepted.
...
12. Embed machine-readable JSON blocks: `research-metadata`, `evidence-index`, `search-log`, `run-status`, `critique-loop-log`, `source-review-findings`, `followup-query-matrix`, `p0p1-closure-matrix`, and `research-decision-gate` when the output affects a high-impact downstream workflow.
...
- High-impact reports are only complete when the recursive closure blocks are internally consistent with `research-decision-gate`.
...
- Read `references/strict-closure-contract.md` for the machine-readable recursive closure contract used by high-impact reports.
...
python scripts/validate_research_process_loop.py <report-html>
...
python scripts/test_validate_research_process_loop.py
```

Success means the HTML is readable, embedded JSON is parseable, recursive P0/P1 closure evidence is present for high-impact reports, decision-gate rules are enforced, and no obvious secrets are present.
