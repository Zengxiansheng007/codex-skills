...
def apply_sync_plan(
    ...
    *,
    ...
    """Re-read inputs, acquire the branch lock, publish and atomically activate."""
    display_root = canonical_display_path(target)
    root = io_path(display_root)
    ...
    with exclusive_file_lock(root / ".locks" / lock_name):
        first_active = _active_build(root)
        if first_active != plan.get("expected_active_build"):
            return _result(plan, "concurrent_update", first_active, no_op=False, issues=["E_SYNC_EXPECTED_ACTIVE_MISMATCH"])
        ...
        if canonical_hash(compiled["identity"]) != plan["input_identity_hash"] or compiled["manifest"]["build_fingerprint"] != plan["build_fingerprint"]:
            return _result(plan, "plan_stale", first_active, no_op=False, issues=["E_SYNC_INPUT_CHANGED"])
        if first_active == plan["build_fingerprint"]:
            return _result(plan, "in_sync", first_active, no_op=True, issues=[])
        ...
        if release.exists():
            ...
            if state["release_integrity"] != "valid" or state["input_sync"] != "in_sync":
                return _result(plan, "validation_failed", first_active, no_op=False, issues=["E_SYNC_EXISTING_RELEASE_INVALID"])
        else:
            ...
            try:
                ...
                return _result(plan, "generation_failed", first_active, no_op=False, issues=[str(exc).split(":", 1)[0]])
        second_active = _active_build(root)
        if second_active != first_active:
            return _result(plan, "concurrent_update", second_active, no_op=False, issues=["E_SYNC_SECOND_CAS_FAILED"])
        pointer = {
        ...
        if _active_build(root) != plan["build_fingerprint"]:
            return _result(plan, "validation_failed", first_active, no_op=False, issues=["E_SYNC_ACTIVE_READBACK_FAILED"])
